...
void dispatcher::init()
{
	for (int i = 0; i != 5; ++i)
	{
		...
		connect((QObject*)lift_threads_[i], SIGNAL(updateArrow(int, int)), this, SLOT(updateArrow(int, int)));
		connect((QObject*)lift_threads_[i], SIGNAL(updateArrow(int, int)), this, SLOT(updateArrow(int, int)));
...
void dispatcher::updateArrow(int unit, int arrow)
{
	if (arrow == -1)
	{
		arrows_[0][unit - 1]->setVisible(false);
		arrows_[1][unit - 1]->setVisible(false);
	}
	else if (arrow == 1)
	{
		arrows_[0][unit - 1]->setVisible(false);
		arrows_[1][unit - 1]->setVisible(true);
	}
	else
	{
		arrows_[0][unit - 1]->setVisible(true);
		arrows_[1][unit - 1]->setVisible(false);
	}
}